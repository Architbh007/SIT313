...
export default function Login() {
  ...
  const session = getSession(); // <— detect if already logged in
  ...
  if (session) {
    return (
      <div style={card}>
        <h2 style={{ textAlign:"center", margin:0, marginBottom:10 }}>You’re already signed in</h2>
        <p style={{ marginBottom: 12 }}>Email: <b>{session.email}</b></p>
        <button
          style={btn}
          onClick={() => { clearSession(); /* stay on /login */ }}
        >
          Sign out
        </button>
  ...
  // Not logged in → show your existing login UI
  return (
    <div style={card}>
